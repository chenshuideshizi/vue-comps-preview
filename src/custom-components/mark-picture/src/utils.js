...
export class AlarmPicture {
  constructor(options) {
    const DEFAULT_OPTIONS = {
      imageWidth: 1920,
      imageHeight: 1080,
      targetColor: 'rgba(248, 81, 81, 1)'
    }
    ...
    this.deviceRegion = this.options.deviceRegion || []
    this.alarmedRegion = this.options.alarmedRegion || []

    this.alarmColor = this.options.alarmColor
    this.targetColor = this.options.targetColor
    ...
    // 绘制报警图片
    if (this.options.alarmPicture) {
      const alarmPicture = this.options.alarmPicture
      ctx.save()
      const imageWidth = alarmPicture.width
      const imageHeight = alarmPicture.height
      ...
      ctx.drawImage(alarmPicture, 0, 0, this.imageWidth, this.imageHeight)
      ctx.restore()
    }

    // 绘制设备布控的区域
    this.renderDeviceRegion()

    // 绘制报警目标的区域
    this.renderAlarmedRegion()
